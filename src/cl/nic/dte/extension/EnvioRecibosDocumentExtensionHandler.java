...
public class EnvioRecibosDocumentExtensionHandler {

	public static byte[] getBytes(EnvioRecibosDocument dte) throws IOException {
	...
	public static VerifyResult verifyXML(EnvioRecibosDocument dte) {
		return XMLUtil.verifyXML(dte);

	}

	public static VerifyResult verifySignature(EnvioRecibosDocument dte) {
		...
		cl.sii.siiDte.dsig.SignatureType sign = dte.getEnvioRecibos()
				.getSignature();
		...
		return (XMLUtil.verifySignature(sign.getDomNode(), dte
				.getEnvioRecibos().getSetRecibos().getCaratula()
				.getTmstFirmaEnv().getTime()));
	}

	public static byte[] sign(EnvioRecibosDocument dte, PrivateKey pKey,
		...
		String uri = "#" + dte.getEnvioRecibos().getSetRecibos().getID();
		// Segun el esquema no estoy obligado a este formato de fecha, pero
		// prefiero

		dte.getEnvioRecibos().getSetRecibos().getCaratula().xsetTmstFirmaEnv(
	...
	public static X509Certificate getCertificate(EnvioRecibosDocument dte) {
		return XMLUtil.getCertificate(dte.getEnvioRecibos().getSignature());
	}
