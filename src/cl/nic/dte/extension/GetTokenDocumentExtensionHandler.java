...
public class GetTokenDocumentExtensionHandler {

	public static byte[] getBytes(GetTokenDocument dte) throws IOException {
	...
	public static VerifyResult verifyXML(GetTokenDocument dte) {
		return XMLUtil.verifyXML(dte);

	}

	public static VerifyResult verifySignature(GetTokenDocument dte) {
		...
		cl.sii.siiDte.dsig.SignatureType sign = dte.getGetToken().getSignature();
		...
		return (XMLUtil.verifySignature(sign.getDomNode()));
	}

	public static byte[] sign(GetTokenDocument dte, PrivateKey pKey,
		...
		XMLUtil.signEmbeded(dte.getGetToken().getDomNode(), uri, pKey, cert);
		XmlOptions opts = new XmlOptions();
	...
	public static X509Certificate getCertificate(GetTokenDocument dte) {
		return XMLUtil.getCertificate(dte.getGetToken().getSignature());
	}
